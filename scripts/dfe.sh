...
xfce4-terminal \
	...
	--tab -T "papi-db" --working-directory=/home/mark/projects/explore-education-statistics -e "bash -c 'trap : INT; docker-compose up public-api-db; exec bash'" \
	--tab -T "papi" --working-directory=/home/mark/projects/explore-education-statistics/src/GovUk.Education.ExploreEducationStatistics..Public.Data.Api -e "bash -c 'source /home/mark/.bashrc; dotnet clean; dotnet build; trap : INT; dotnet run; cat ~/dfe.sh | grep data-pro; exec bash'" \
	--tab -T "papi-processor" --working-directory=/home/mark/projects/explore-education-statistics/src/GovUk.Education.ExploreEducationStatistics.Public.Data.Processor -e "bash -c 'source /home/mark/.bashrc; dotnet clean; dotnet build; trap : INT; func host start --port 7074 --pause-on-error; cat ~/dfe.sh | grep data-pro; exec bash'" \
